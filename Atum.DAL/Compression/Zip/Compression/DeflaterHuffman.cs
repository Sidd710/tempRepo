...
namespace Atum.DAL.Compression.Zip.Compression 
{
	...
	public class DeflaterHuffman
	{
		...
		class Tree 
		{
			...
			public void CheckEmpty()
			{
				...
				for (int i = 0; i < freqs.Length; i++) {
					if (freqs[i] != 0) {
						//Debug.WriteLine("freqs[" + i + "] == " + freqs[i]);
						empty = false;
					}
				}
			...
			public void BuildCodes() 
			{
				...
				//				if (DeflaterConstants.DEBUGGING) {
				//					//Debug.WriteLine("buildCodes: "+freqs.Length);
				//				}
				...
				for (int bits = 0; bits < maxLength; bits++) {
					...
					//					if (DeflaterConstants.DEBUGGING) {
					//						//Debug.WriteLine("bits: " + ( bits + 1) + " count: " + bl_counts[bits]
					//						                  +" nextCode: "+code);
					//					}
				}
				...
				for (int i=0; i < numCodes; i++) {
					...
					if (bits > 0) {
						...
						//						if (DeflaterConstants.DEBUGGING) {
						//								//Debug.WriteLine("codes["+i+"] = rev(" + nextCode[bits-1]+"),
						//								                  +bits);
						//						}
			...
			void BuildLength(int[] childs)
			{
				...
				//				if (DeflaterConstants.DEBUGGING) {
				//					//Debug.WriteLine("Tree "+freqs.Length+" lengths:");
				//					for (int i=0; i < numLeafs; i++) {
				//						//Debug.WriteLine("Node "+childs[2*i]+" freq: "+freqs[childs[2*i]]
				...
				*/
				...
				//				if (DeflaterConstants.DEBUGGING) {
				//					//Debug.WriteLine("*** After overflow elimination. ***");
				//					for (int i=0; i < numLeafs; i++) {
				//						//Debug.WriteLine("Node "+childs[2*i]+" freq: "+freqs[childs[2*i]]
				//						                  + " len: "+length[childs[2*i]]);
				//					}
...
#if DebugDeflation
			//			if (DeflaterConstants.DEBUGGING) {
			//				//Debug.WriteLine("Flushing stored block "+ storedLength);
			//			}
#endif
		...
		public void FlushBlock(byte[] stored, int storedOffset, int storedLength, bool lastBlock)
		{
			...
			if (storedOffset >= 0 && storedLength + 4 < opt_len >> 3) {
				...
				//				if (DeflaterConstants.DEBUGGING) {
				//					//Debug.WriteLine("Storing, since " + storedLength + " < " + opt_len
				//					                  + " <= " + static_len);
				//				}
